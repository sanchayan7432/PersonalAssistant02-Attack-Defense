...
from SanchayanGhosh001.SanchayanGhosh_Folder01.PersonalAssistant02.attack_module_01 import AttackModule
...
def reinforcement_loop(api_key=None, iterations=100, log_path="data/logs2.csv"):
    ...
    attack_module = AttackModule(api_key="AIzaSyDDKvpe02p6R-fYL0lCUDNBLWJRiBR4B78")
    defense_module = DefenseModule()
    reward_module = RewardModule()

    total_attempts = 0
    successful_defenses = 0  # count of times leak was prevented

    with open(log_path, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([
            "Iteration", "Attack Query", "Defense Response", 
            "Leak Successful", "Anomaly Score", "Reward", "Loss", "Threshold", "Accuracy"
        ])
        ...
        for i in range(iterations):
            ...
            if blocked:
                ...
                model_response = defense_response  # defense message shown
            else:
                model_response = attack_module.attack(attack_query)
                leak_keywords = ["system", "prompt", "internal", "config", "instruction", "logic", "role", "configuration"]
            ...
            time.sleep(5)  # <-- 5 second delay before next iteration
